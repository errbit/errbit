...
RSpec.describe Errbit::UserPolicy do
  ...
  it "exposes the permitted FIELDS list" do
  ...
  describe "#permitted_attributes" do
    context "when user is an admin" do
      context "and record is the same admin user" do
        let(:user) { create(:errbit_user, admin: true) }
        let(:record) { user }
      ...
      context "and record is a different user" do
        let(:user) { create(:errbit_user, admin: true) }
        let(:record) { create(:errbit_user, admin: false) }
...
RSpec.describe Errbit::UserPolicy::Scope do
  describe "#resolve" do
    context "when user is an admin" do
      ...
      it { expect(subject.resolve).to match_array([admin, user]) }
    end
