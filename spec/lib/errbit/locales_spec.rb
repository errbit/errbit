...
RSpec.describe Errbit::Locales do
  ...
  it "discovers the Errbit UI catalogs" do
    expect(described_class.selectable.map(&:identifier)).to eq(["ca", "en", "es", "eu", "pt-BR"])
  end

  it "returns catalog autonyms" do
    expect(described_class.selectable.map(&:name)).to eq(["Català", "English", "Español", "Euskara", "Português (Brasil)"])
  end
