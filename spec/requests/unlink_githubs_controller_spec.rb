...
RSpec.describe UnlinkGithubsController, type: :request do
  describe "#update" do
    context "when user is logged in" do
      context "when user unlinks their own GitHub account" do
        ...
        it "clears github_login and github_oauth_token and redirects to user_path" do
          expect(response).to redirect_to(user_path(assigns(:user)))
          expect(response).to have_http_status(:found)
          expect(assigns(:user).github_login).to be_nil
          expect(assigns(:user).github_oauth_token).to be_nil
        end
      end

      context "when user is not an admin and unlinks another user's GitHub account" do
