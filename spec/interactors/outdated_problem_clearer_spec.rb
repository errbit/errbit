...
RSpec.describe OutdatedProblemClearer do
  describe "#execute" do
    ...
    context "when ERRBIT_PROBLEM_DESTROY_AFTER_DAYS env is not set" do
      it "do nothing" do
        expect { subject.execute }.not_to change(Problem, :count)
      end
    end

    context "when ERRBIT_PROBLEM_DESTROY_AFTER_DAYS env is set" do
      before do
        expect(Rails.configuration.errbit)
          .to receive(:notice_deprecation_days)
          .and_return(7)
          .at_least(:once)
      end
