...
RSpec.describe Errbit::BacktraceLineDecorator, type: :decorator do
  ...
  let(:app) { create(:errbit_app, github_repo: "foo/bar", repository_branch: "main") }
  ...
  context "with string-keyed hash (post-JSON deserialization)" do
    let(:line) do
      described_class.new(
        "number" => 884,
        "file" => "[PROJECT_ROOT]/path/to/file/ea315ea4.rb",
    ...
    it "still reads number, file, and method" do
      expect(line.number).to eq(884)
      expect(line.file).to eq("[PROJECT_ROOT]/path/to/file/ea315ea4.rb")
