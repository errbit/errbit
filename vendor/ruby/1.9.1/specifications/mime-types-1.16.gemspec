# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = "mime-types"
  s.version = "1.16"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = ["Austin Ziegler"]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"]
  s.date = "2009-03-02"
  s.description = "MIME::Types for Ruby originally based on and synchronized with MIME::Types for Perl by Mark Overmeer, copyright 2001 - 2009. As of version 1.15, the data format for the MIME::Type list has changed and the synchronization will no longer happen."
  s.email = ["austin@rubyforge.org"]
  s.extra_rdoc_files = ["History.txt", "Install.txt", "Licence.txt", "README.txt"]
  s.files = ["History.txt", "Install.txt", "Licence.txt", "README.txt"]
  s.homepage = "http://mime-types.rubyforge.org/"
  s.rdoc_options = ["--main", "README.txt"]
  s.require_paths = ["lib"]
  s.rubyforge_project = "mime-types"
  s.rubygems_version = "1.8.15"
  s.summary = "Manages a MIME Content-Type database that will return the Content-Type for a given filename."

  if s.respond_to? :specification_version then
    s.specification_version = 2

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_development_dependency(%q<archive-tar-minitar>, ["~> 0.5"])
      s.add_development_dependency(%q<nokogiri>, ["~> 1.2"])
      s.add_development_dependency(%q<rcov>, ["~> 0.8"])
      s.add_development_dependency(%q<hoe>, [">= 1.8.3"])
    else
      s.add_dependency(%q<archive-tar-minitar>, ["~> 0.5"])
      s.add_dependency(%q<nokogiri>, ["~> 1.2"])
      s.add_dependency(%q<rcov>, ["~> 0.8"])
      s.add_dependency(%q<hoe>, [">= 1.8.3"])
    end
  else
    s.add_dependency(%q<archive-tar-minitar>, ["~> 0.5"])
    s.add_dependency(%q<nokogiri>, ["~> 1.2"])
    s.add_dependency(%q<rcov>, ["~> 0.8"])
    s.add_dependency(%q<hoe>, [">= 1.8.3"])
  end
end
