...
module Errbit
  class IssueTracker < ApplicationRecord
    ...
    delegate :configured?, :create_issue, :close_issue, :url, to: :tracker
    ...
    def tracker
      @tracker ||= begin
        ...
        klass.new(options.merge(github_repo: app.try(:github_repo), bitbucket_repo: app.try(:bitbucket_repo)))
      end
    end

    def validate_tracker
      (tracker.errors || {}).each do |key, value|
