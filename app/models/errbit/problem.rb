  ...
  class Problem < ApplicationRecord
    ...
    def recache
      ...
      CACHED_NOTICE_ATTRIBUTES.each do |attr, source|
        ...
        send(:"#{attr}=", counts.each_with_object({}) { |(key, count), hash| hash[key] = {"value" => values[key], "count" => count} })
      end
    ...
    def grouped_notice_counts(since, group_by = "day")
      ...
      Errbit::Notice
        ...
        .pluck(:created_at)
        .group_by { |time| time.send(time_method) }
        .map { |bucket, times| {"_id" => {group_by => bucket}, "count" => times.size} }
    ...
    def zero_filled_grouped_noticed_counts(since, group_by = "day")
      ...
      Array.new(buckets) do |i|
        bucket_time = (since + i.send(group_by)).send(time_method)
    ...
    def grouped_notice_count_relative_percentages(since, group_by = "day")
      counts = zero_filled_grouped_noticed_counts(since, group_by).map { |hash| hash.values.first }
      max = counts.max
      counts.map { |count| max.zero? ? 0 : (count.to_f / max.to_f) * 100.0 }
    end
