...
module Errbit
  class Issue
    ...
    def close
      ...
      tracker.errors.each { |key, error| errors.add key, error }
      return false if errors.present?

      issue_tracker.close_issue(problem.issue_link, user: user.attributes) if issue_tracker.respond_to?(:close_issue)
      errors.empty?
    rescue => e
    ...
    def save
      ...
      url = issue_tracker.create_issue(title, body, user: user.attributes)
