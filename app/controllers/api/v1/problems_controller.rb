class Api::V1::ProblemsController < ApplicationController
  ...
  def index
    query = {}
    fields = %w{app_id app_name environment message problems.where first_notice_at last_notice_at resolved resolved_at notices_count}
    ...
    if params.key?(:start_date) && params.key?(:end_date)
      ...
      query = ["first_notice_at <= ? AND (resolved_at IS NULL OR resolved_at >= ?)", end_date, start_date]
    end

    results = benchmark("[api/v1/problems_controller] query time") do
      Problem.where(query).select(fields).to_a
    end
