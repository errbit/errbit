...
class AppsController < ApplicationController
  ...
  helper_method :app_decorate, :apps, :app, :problems, :users, :all_errs, :params_sort, :params_order, :selected_problems, :selected_problems_ids, :err_ids
  ...
  def apps
    @apps ||= begin
      ...
      scope.to_a.sort.map { |record| Errbit::AppDecorator.new(record) }
    end
  end
  ...
  def users
    @users ||= Errbit::User.all.sort_by { |user| user.name.downcase }
  end
  ...
  def selected_problems_ids
    selected_problems.map { |problem| problem.id.to_s }
  end
