...
module Errbit
  class BacktraceLineDecorator < Draper::Decorator
    ...
    def in_app?
      ...
      file.match Errbit::Backtrace::IN_APP_PATH
    end
    ...
    def file
      (object.try(:[], :file) || object.try(:[], "file")).to_s
    end
    ...
    def file_relative
      file.to_s.sub(Errbit::Backtrace::IN_APP_PATH, EMPTY_STRING)
    end
    ...
    def to_s
      column = object.try(:[], :column) || object.try(:[], "column")

      suffix = column.present? ? ":#{column}" : nil
      "#{file_relative}:#{number}#{suffix}"
    end
    ...
    def decorated_path
      path
        .sub(Errbit::Backtrace::IN_APP_PATH, "")
        .sub(Errbit::Backtrace::GEMS_PATH, "<strong>\\1</strong>")
    end
    ...
    def link_to_issue_tracker_file(app, text = nil)
      return unless app.issue_tracker&.respond_to?(:url_to_file)
